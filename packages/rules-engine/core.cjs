...
function resolveWorldOrbit(track, policy = {}) {
  ...
  if (safeTrack.rotationScore >= nextPolicy.promotionScoreFloorOrbit1) return WORLD_ORBITS.ORBIT_3;
...
function ingestTrackForWorld(track = {}) {
  ...
  return {
    ...safeTrack,
    worldOrbit: WORLD_ORBITS.ORBIT_1,
    globalOrbit: null,
...
function evaluateTrackMovement(track, telemetry = [], policy = {}) {
  ...
  else if (previousOrbit === WORLD_ORBITS.ORBIT_1 && nextOrbit === WORLD_ORBITS.ORBIT_2) movement = "promotion";
  else if (previousOrbit === WORLD_ORBITS.ORBIT_2 && nextOrbit === WORLD_ORBITS.ORBIT_3) movement = "promotion";
  else if (previousOrbit === WORLD_ORBITS.ORBIT_3 && nextOrbit === WORLD_ORBITS.ORBIT_2) movement = "demotion";
  else if (previousOrbit === WORLD_ORBITS.ORBIT_2 && nextOrbit === WORLD_ORBITS.ORBIT_1) movement = "demotion";
...
function evaluateSessionSummary({
  ...
  const updatedTracks = (tracks || []).map((track) => {
    ...
    const nextTrack = {
      ...
      orbit:
        movementResult.worldOrbit === WORLD_ORBITS.ORBIT_3
          ? "rotation"
          : movementResult.worldOrbit === WORLD_ORBITS.ORBIT_2
            ? "recent"
            : "discovery",
      rotation: movementResult.worldOrbit === WORLD_ORBITS.ORBIT_3 || Boolean(track.rotationOverride === "force_on"),
