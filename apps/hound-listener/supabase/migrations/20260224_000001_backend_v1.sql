...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'releases_cover_asset_fk'
  ) then
    alter table releases
      add constraint releases_cover_asset_fk
...
begin
  if not exists (
    select 1 from pg_constraint where conname = 'tracks_master_asset_fk'
  ) then
    alter table tracks
      add constraint tracks_master_asset_fk
